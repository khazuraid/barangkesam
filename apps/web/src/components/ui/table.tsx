...
const TableRow = React.forwardRef<HTMLTableRowElement, React.HTMLAttributes<HTMLTableRowElement>>(
  ({ className, ...props }, ref) => (
    <tr
      ref={ref}
      className={cn(
        'border-b transition-colors hover:bg-slate-50/70 data-[state=selected]:bg-blue-50',
        className,
      )}
      {...props}
    />
...
>(({ className, ...props }, ref) => (
  <th
    ref={ref}
    className={cn(
      'h-11 px-3 text-left align-middle font-semibold text-slate-600 text-xs uppercase tracking-wide [&:has([role=checkbox])]:pr-0',
      className,
    )}
    {...props}
  />
